...
class SystemState:
    ...
    def set_mode(self, mode, target=None, request_id=None):
        with self.lock:
            self.mode = mode
            self.target_object = target
            self.current_request_id = request_id
            print(f"[STATE] Mode changed to {mode}, Target: {target}, ReqID: {request_id}")

state = SystemState()

def format_ws_message(message_dict):
...
def broadcast_ws_message(message_dict):
    ...
    if not state.clients or not state.loop:
        return
    message_dict = format_ws_message(message_dict)
    ...
    try:
        async for message in websocket:
            ...
            try:
                ...
                async def send_reply(reply_dict):
                    reply_dict = format_ws_message(reply_dict)
                ...
                if cmd_type == "find_object" or cmd_type == "FIND_OBJECT":
                    ...
                    if target and target in CLASS_NAMES:
                        state.set_mode("FIND_OBJECT", target, request_id=req_id)
                        await send_reply({"type": "ack", "payload": {"message": f"Searching for {target}"}})
                    else:
                        await send_reply({"type": "pi_error", "payload": {"error_code": "UNKNOWN_OBJECT", "message": "Unknown object"}})
                
                elif cmd_type == "describe_scene" or cmd_type == "DESCRIBE_FRONT":
                    state.set_mode("DESCRIBE", request_id=req_id)
                    await send_reply({"type": "ack", "payload": {"message": "Describing scene"}})
                    
                elif cmd_type == "ping":
                    ping_id = data.get("payload", {}).get("ping_id", "")
                    await send_reply({"type": "pong", "payload": {"ping_id": ping_id}})
                    
                elif cmd_type == "STOP" or cmd_type == "stop":
                    state.set_mode("IDLE")
                    await send_reply({"type": "ack", "payload": {"message": "Stopped search"}})
                    ...
                    if state.tof_manager and hasattr(state.tof_manager, 'trigger_fake_warning'):
                        ...
                        await send_reply({"type": "ack", "payload": {"message": "Simulated ToF obstacle!"}})
                    ...
                    await send_reply({"type": "ack", "payload": {"message": "Vision AI paused"}})
                    
                elif cmd_type == "resume_vision":
                    state.set_mode("IDLE")
                    await send_reply({"type": "ack", "payload": {"message": "Vision AI resumed"}})
